...
import { parseResume } from '../../../api/ats-candidates';
...
export default function CandidateForm({ initial, onSubmit, onCancel }: Props) {
  ...
  const handleParseResume = async () => {
    ...
    try {
      ...
      const filled: string[] = [];
      if (parsed.name)   { setForm((p) => ({ ...p, name:   parsed.name   })); filled.push('Name'); }
      if (parsed.mobile) { setForm((p) => ({ ...p, mobile: parsed.mobile })); filled.push('Mobile'); }
      if (parsed.education.length)       { setEduRows(parsed.education);       filled.push('Education'); }
      if (parsed.work_experience.length) { setExpRows(parsed.work_experience); filled.push('Work Experience'); }
      setParseMsg(filled.length ? `Auto-filled: ${filled.join(', ')}` : 'No data could be extracted — fill in manually.');
    } catch (err: any) {
      setParseMsg(err.message ?? 'Failed to parse resume');
    } finally {
      setParsing(false);
    }
  };
